...
<style>
    .s-cphover{
        background: url(<?php echo ($SiteInfo["pic_hover"]); ?>) no-repeat;
    }
</style>
<!--中间开始-->
<div class="main">
    <!-------------------------banner 开始-------------------------------->
    <div class="zy_banner"><img src="./Public/Uploads/images/jidi.jpg" width="1004" height="310"/></div>
    <!-------------------------banner 结束-------------------------------->
    <div class="w1002">
        <div class="s-zytop">
            <ul class="clearfix">
            ...
            <div class="cplist">
                <ul class="clearfix">
                    <?php if(is_array($pictures)): foreach($pictures as $key=>$val): ?><li>
                            <a class="cpimg" href="<?php echo U('Category/base', array('id'=>$val['id']));?>">
                                <img src="<?php echo ($val["pic"]); ?>" width="300" height="300" />
                            </a>
                            <h3><a href="<?php echo U('Category/base', array('id'=>$val['id']));?>" ><?php echo ($val["title"]); ?></a></h3>
                            <a class="ckxx" href="<?php echo U('Category/base', array('id'=>$val['id']));?>" >了解详情</a>
                            <a href="<?php echo U('Category/base', array('id'=>$val['id']));?>" class="s-cphover" ></a>
...
<script type="text/javascript">
    $(function () {
        $(".cplist li").hover(function () {
            ...
            $(this).children(".s-cphover").stop(false, true).animate({ 'top': 0 }, 300);
        }, function () {
            $(this).removeClass("cp-hover");
            $(this).children(".s-cphover").stop(false, true).animate({ 'top': -182 }, 300);
        });
    });
</script>